...
from vers import ozw_vers_major, ozw_vers_minor, ozw_vers_revision
#from libc.stdlib cimport malloc, free
#from mylibc cimport PyEval_InitThreads
from node import NodeData #NodeData_t,
from node import SecurityFlag
from zwemulator.lib.defs import *
from zwemulator.lib.driver import DriverData #DriverData_t, 
from zwemulator.lib.driver import ControllerCommand, ControllerState, ControllerError #, pfnControllerCallback_t
from zwemulator.lib.notification import Notification, NotificationType, NotificationCode
#from zwemulator.lib.notification import Type_Notification, Type_Group, Type_NodeEvent
#from zwemulator.lib.notification import Type_CreateButton, Type_DeleteButton, Type_ButtonOn, Type_ButtonOff
#from zwemulator.lib.notification import Type_SceneEvent
#from zwemulator.lib.notification import const_notification, pfnOnNotification_t
from zwemulator.lib.values import ValueGenre, ValueType, ValueID
from options import Options #, Create
from manager import Manager #, Create, Get
...
def getValueFromType(manager, valueId):
    """
    Translate a value in the right type
    """
    c = ""
    ret = None
    if valueId in values_map:
        datatype = PyValueTypes[values_map[valueId].GetType()]
        if datatype == "Bool":
            cret, type_bool  = manager.GetValueAsBool(values_map[valueId])
            ret = type_bool if cret else None
            return ret
        elif datatype == "Byte":
            cret, type_byte = manager.GetValueAsByte(values_map[valueId])
            ret = type_byte if cret else None
            return ret
        elif datatype == "Raw":
            cret, vectraw, size = manager.GetValueAsRaw(values_map[valueId])
            if cret:
            ...
            cret,  type_float = manager.GetValueAsFloat(values_map[valueId])
            ret = type_float if cret else None
            return ret
        elif datatype == "Int":
            cret,  type_int = manager.GetValueAsInt(values_map[valueId])
            ret = type_int if cret else None
            return ret
        elif datatype == "Short":
            cret, type_short = manager.GetValueAsShort(values_map[valueId])
            ret = type_short if cret else None
            return ret
        elif datatype == "String":
            cret, type_string= manager.GetValueAsString(values_map[valueId])
            ret = type_string if cret else None
            return ret
            ...
            cret, type_string = manager.GetValueListSelection(values_map[valueId])
            ret = type_string if cret else None
            return ret
        else :
            cret, type_string = manager.GetValueAsString(values_map[valueId])
...
def ctrl_callback(_state, _error, callback) :
    """
...
class PyOptions:
    ...
    def create(self, a, b, c):
        ...
        """
        self.options = Options(str(a), str(b), str(c))
    ...
    def addOptionBool(self, name, value):
        ...
        """
        return self.options.AddOptionBool(str(name), value )
    ...
    def addOptionInt(self, name, value):
        ...
        """
        return self.options.AddOptionInt(str(name), value )
    ...
    def addOptionString(self, name, value, append):
        ...
        """
        return self.options.AddOptionString(str(name), str(value), append )
...
:see: removeDriver_
        ...
        '''
        self.manager.AddDriver(serialport)
...
Retrieve statistics from zwemulator.lib.driver.
...
:see: removeWatcher_
        ...
        if not self.manager.AddWatcher(notif_callback, pythonfunc):
...
:see: addWatcher_
        ...
        '''
        if not self.manager.RemoveWatcher(notif_callback, self._watcherCallback):
...
:see: cancelControllerCommand_
        ...
        return self.manager.BeginControllerCommand(homeId, command, \
                 ctrl_callback, pythonfunc, highPower, nodeId, arg)
...
#        cdef vector[ValueID] vect
    ...
    def addSceneValue(self, sceneid, id, value):
        '''
.. _addSceneValue:
...
#        cdef string type_string
    ...
    def removeSceneValue(self, sceneid, id):
        '''
.. _removeSceneValue:
...
sceneGetValues_
    ...
    def setSceneValue(self, sceneid, id, value):
        '''
.. _setSceneValue:
